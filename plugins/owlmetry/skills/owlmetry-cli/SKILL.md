...
# Metrics
owlmetry metrics list (--project-id <id> | --team-id <id>) --format json  # --team-id lists every metric across accessible projects in the team
...
# Funnels
owlmetry funnels list (--project-id <id> | --team-id <id>) --format json  # --team-id lists every funnel across accessible projects in the team
...
owlmetry questionnaires list (--project-id <id> | --team-id <id>) [--app-id <id>] [--active | --inactive] [--data-mode production|development|all] [--limit <n>] --format json  # --team-id lists every questionnaire across accessible projects
...
owlmetry questionnaires view <questionnaireId> --project-id <id> [--data-mode production|development|all] --format json
...
# Advertising insights (campaigns/ad-groups/keywords+ads ranked by lifetime USD revenue, with spend + ROAS)
...
owlmetry ads sync --project-id <id> --format json   # admin-only; fires revenuecat_sync + apple_ads_sync (the latter refreshes both spend metrics and IDs→names), returns both job_run_ids

# Events
owlmetry events [--project-id <id>] [--app-id <id>] [--level <levels>] [--user-id <id>] [--session-id <id>] [--since <time>] [--limit <n>] [--order asc|desc] --format json
...
```bash
owlmetry metrics list --project-id <id> --format json                   # List for one project
owlmetry metrics list --team-id <id> --format json                      # List across every accessible project in the team
...
```bash
owlmetry funnels list --project-id <id> --format json                   # List for one project
owlmetry funnels list --team-id <id> --format json                      # List across every accessible project in the team
...
```bash
owlmetry events [--project-id <id>] [--app-id <id>] [--since <time>] [--until <time>] [--level <levels>] [--user-id <id>] [--session-id <id>] [--screen-name <name>] [--limit <n>] [--cursor <cursor>] [--data-mode production|development|all] [--order asc|desc] --format json
owlmetry events view <id> --format json
```
...
`--level` accepts a single level (`--level error`) or a comma-separated list (`--level warn,error`). Allowed values: `info`, `debug`, `warn`, `error`.
...
Rank attributed users by lifetime USD revenue, broken down by campaign → ad group → keyword | ad, with **ad spend + ROAS** alongside revenue. The hierarchy is generic over `attribution_source` — today only `apple_search_ads` is shipped (the namespace reserves `meta`/`google-ads`/`tiktok` for later). The data combines three feeds: ASA campaign/ad-group/keyword/ad **names** + **spend** come from the Apple Search Ads integration's `apple_ads_sync` job (which now syncs both ID→name resolution AND spend metrics); lifetime **revenue** comes from RevenueCat (`total_revenue_usd_cents` on `app_users`, refreshed by the `revenuecat_sync` job). A user only contributes revenue once their RC subscriber data has been synced and their attribution IDs have been resolved to names.

Each row carries `user_count`, `paid_user_count` (lifetime — anyone who has ever paid, surfaced as the "Conv." column), `retained_user_count` (current — users on an auto-renewing paid subscription right now, excluding trials and cancelled-but-still-in-period; surfaced as the "Retained" column), `total_revenue_usd`, `arpu`, plus `total_spend_usd` and `roas` (null when no integration / no spend data). The `campaigns` response also returns project-wide `total_*` rollups (`total_user_count`, `total_paid_user_count`, `total_retained_user_count`, `total_revenue_usd`, `total_spend_usd`) and two staleness timestamps: `revenue_synced_at` (latest RC sync across the project's users) and `ad_metrics_synced_at` (latest ASA spend sync) — null when never synced. Non-USD ASA orgs surface a `currency_warning` (the org's reporting currency) and `roas` stays null because the conversion isn't done server-side. The CLI table prints `Users · Conv. · Retained · Revenue · ARPU [· Spend · ROAS]`; the summary line above it reads `<N> attributed users · <N> conversions · <N> retained · <revenue> lifetime revenue [· <spend> spend (ROAS <x>)]`.
